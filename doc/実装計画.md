...
- [x] フロントエンド：カート画面（Claude Code担当）
  - [x] `api/cart.ts` - カートAPI
  - [x] `api/types.ts` - カート型定義
  - [x] `stores/cart.ts` - カートストア（Pinia）
  - [x] `views/cart/CartView.vue` - カート画面
  - [x] `router/index.ts` - カートルート追加
  - [x] `components/AppHeader.vue` - カートバッジ表示
  - [x] `views/products/ProductDetailView.vue` - 「カートに追加」ボタン

- [x] 動作確認
  - [x] カート追加 API テスト
  - [x] カート取得 API テスト
  - [x] 数量更新 API テスト（楽観的ロック + リトライ）
  - [x] カート削除 API テスト
...
- `frontend/src/views/cart/CartView.vue` ✅
...
- [x] 注文確定トランザクション実装
  ...
  - [x] 在庫減算（条件付き）
  - [x] カートクリア
  - [x] 全操作を TransactWriteItems で実行
- [x] トランザクションエラーハンドリング
  - [x] ConditionalCheckFailed（在庫不足）
  - [x] TransactionConflict（競合）
- [x] 注文履歴機能
  - [x] Query でユーザーの注文一覧
  - [x] Query で注文明細取得
- [x] フロントエンド：注文確認・履歴画面
...
- [x] 価格履歴機能
  - [x] 価格更新時に履歴レコード作成
  - [x] SK=`PRICE#<timestamp>` で時系列保存
  - [x] BETWEEN クエリで範囲取得
- [x] 在庫変動ログ
  - [x] 在庫変更時に履歴レコード作成
  - [x] SK=`INVLOG#<timestamp>` で時系列保存
- [x] ページネーション実装
  - [x] Limit + LastEvaluatedKey
- [x] フロントエンド：価格履歴グラフ（Chart.js）
- [x] 管理画面：在庫変動履歴
...
- [x] Lambda 関数作成（Go）
  - [x] DynamoDB Streams イベント処理
  - [x] 在庫変動の検知
  - [x] 閾値以下で LOW STOCK ALERT
- [x] Streams トリガー設定
  - [x] イベントソースマッピング
- [x] Lambda デプロイ
  - [x] IAM ロール作成
...
- フロントエンド実装完了（Claude Code担当）
  - api/cart.ts, stores/cart.ts
  - views/cart/CartView.vue（カート画面）
  - AppHeader.vue（カートバッジ）
  - ProductDetailView.vue（カートに追加ボタン）
- cart_repo.go に ConditionExpression 追加（修正）
- 動作確認完了（カートCRUD + 楽観的ロック）
- 楽観的ロックの学習ポイント:
  - ConditionExpression: "version = :currentVer"
  - ConditionalCheckFailedException で競合検知
  - リトライ: 最新バージョン取得 → 再試行（サービス層で実装）
  - リトライ: 最新バージョン取得 → 再試行
```
...
完了日: 2026-05-01
メモ:
- lambda/inventory-stream-handler/main.go 実装完了
  - events.DynamoDBEvent でストリームイベントを受信
  - PRODUCT#xxx / METADATA レコードかつ MODIFY イベントのみ処理
  - OldImage と NewImage の stock を比較
  - 閾値（20）超え → 以下になった瞬間に LOW STOCK ALERT
  - 在庫0 で OUT OF STOCK アラート
- infrastructure/scripts/setup-lambda.sh 実装完了
  - IAM ロール作成（AWSLambdaDynamoDBExecutionRole 付与）
  - GOOS=linux GOARCH=amd64 で bootstrap バイナリをビルド
  - provided.al2023 ランタイム（Custom Runtime）で Lambda 関数を作成/更新
  - DynamoDB Streams のイベントソースマッピング作成（StartingPosition=LATEST, BatchSize=10）
- 動作確認完了
  - aws dynamodb update-item で stock を直接更新 → CloudWatch Logs にアラート出力確認
- DynamoDB Streams の学習ポイント:
  - StreamViewType=NEW_AND_OLD_IMAGES で変更前後の値を取得可能
  - INSERT/MODIFY/REMOVE の3種のイベント
  - events.DynamoDBAttributeValue は Number() メソッドで文字列として値を取得（要 strconv.Atoi）
  - Go Lambda は Custom Runtime（provided.al2023）+ bootstrap バイナリが現在のベストプラクティス
  - Event Source Mapping により Lambda 側がポーリングで Streams を読み取る
- ハマりポイント:
  - SSO トークン期限切れで login が 401 → Login ハンドラが全エラーを 401 に丸めていた（要改善）
  - go build ./... で macOS バイナリが残り gitignore 漏れ → lambda/*/inventory-stream-handler を追加
```
